...
const persistence = {
	...
	onSave(data, storagePreference = null) {
		// Prepare the data to store
		let dataToStore = this.prepareDataToSave(data, storagePreference);

		// Save data to any preferred persistence store
		(0, _persistence.saveData)(this.storeKey, dataToStore, storagePreference);

		// Once saving is done, re render the account list...
		_emitter2.default.emit('renderAccountList', dataToStore);
	},

	onLoad(storagePreference = null) {
		_emitter2.default.emit('onAccountList', (0, _persistence.loadData)(this.storeKey, storagePreference));
	},

	onDelete(id, storagePreference = null) {
		let accountList = (0, _persistence.loadData)(this.storeKey, storagePreference);

		if (!accountList[id]) {
			return;
		}

		let accountToDelete = accountList[id];

		accountList.splice(id, 1);

		(0, _persistence.saveData)(this.storeKey, accountList, storagePreference);

		_emitter2.default.emit('onRemoved', accountToDelete);

		_emitter2.default.emit('renderAccountList', accountList);
	},

	prepareDataToSave(data, storagePreference) {
		let emailAddress = data.emailAddress,
		    password = data.password,
		    accountType = data.accountType,
		    country = data.country,
		    emailPrefix = emailAddress.split('-')[0];


		_formFieldsUtil2.default.keepFormFields(data, ['accountNumber', 'emailAddress', 'flowUrl']);

		data.key = `${emailPrefix}_${accountType}_${country}`;

		// Get existing data
		let existingData = (0, _persistence.loadData)(this.storeKey, storagePreference);
...
module.exports = {
	onSave: persistence.onSave.bind(persistence),
	onLoad: persistence.onLoad.bind(persistence),
	onDelete: persistence.onDelete.bind(persistence)
};
